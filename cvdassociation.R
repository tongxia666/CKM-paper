...
x <- c(
  ...
  131298, 131300, 131302, 131304, 131306, 131360, 131362, 131366, 131368, 131354, 
131364,  131296, 131058, 131056, 131386, 131350, 132032, 21842, 40000, 22189, "f.eid", 191, 42000, 42006, 42008, 42010)

file_output <- "/n/home_fasse/txia/UKB_CKM/Data/cvdvariable"
csv_output <- "/n/home_fasse/txia/UKB_CKM/Data/cvdvariable.csv"
...
# Load the dataset
cvd_association <- fread("/n/home_fasse/txia/UKB_CKM/Data/1_vs_0_combined_training_optimization_testing.csv")
ukb_data <- fread("/n/home_fasse/txia/UKB_CKM/Data/cvdvariable.csv")

cvd_association$sbp_use <- ifelse(!is.na(cvd_association$f.4080.0.0.x), cvd_association$f.4080.0.0.x, cvd_association$f.93.0.0.x)
summary(cvd_association$sbp_use)

table(cvd_association$sex.x)
table(cvd_association$sex.y)


# Remove rows with missing 'ckm'
cvd_association <- cvd_association[!is.na(cvd_association$ckm), ] 

summary(cvd_association$protein_score)
sd(cvd_association$protein_score, na.rm = TRUE)

# Standardize protein_score to mean = 0, SD = 1
#cvd_association[, protein_score := scale(protein_score)]
cvd_association[, protein_score := qnorm(rank(protein_score, na.last = "keep") / (.N + 1))]
cvd_association[, protein_score_0.2 := qnorm(rank(protein_score_0.2, na.last = "keep") / (.N + 1))]

summary(cvd_association$protein_score)
sd(cvd_association$protein_score, na.rm = TRUE)


summary(cvd_association$protein_score_0.2)
sd(cvd_association$protein_score_0.2, na.rm = TRUE)




# 1. Basic summary
summary_stats <- summary(cvd_association$protein_score)
...
# 3. Identify outliers
outliers <- cvd_association$protein_score[
  cvd_association$protein_score < lower_bound |
  cvd_association$protein_score > upper_bound
]

# 4. Compute 1st and 99th percentiles
p1 <- quantile(cvd_association$protein_score, 0.01, na.rm = TRUE)
p99 <- quantile(cvd_association$protein_score, 0.99, na.rm = TRUE)
...
# Define CVD-related outcomes (exclude CKD-related outcomes)
CVD_outcomes <- c(
  "f.131298.0.0",  # Acute myocardial infarction
  "f.131300.0.0",  # Subsequent myocardial infarction
  "f.131302.0.0",  # Complications following acute myocardial infarction
  "f.131304.0.0",  # Other acute ischaemic heart diseases
  "f.131306.0.0",  # Chronic ischaemic heart disease
  "f.131360.0.0",  # Subarachnoid haemorrhage
  "f.131362.0.0",  # Intracerebral haemorrhage
  "f.131366.0.0",  # Cerebral infarction
  "f.131368.0.0",  # Stroke, not specified as haemorrhage or infarction
  "f.131354.0.0",  # Heart failure
  "f.131296.0.0",  # Angina pectoris
  "f.131364.0.0",  # Other cerebrovascular diseases
  "f.131058.0.0",  # Transient ischaemic attack
  "f.131056.0.0",  # Vascular syndromes of brain
  "f.131386.0.0",  # Other peripheral vascular diseases
  "f.131350.0.0",   # Atrial fibrillation and flutter
"f.42000.0.0", "f.42006.0.0", "f.42008.0.0", "f.42010.0.0"
)
...
# Step 1: Identify first CVD diagnosis date and handle Inf values
...
# Step 2: Determine baseline CVD (prevalent cases: diagnosis before baseline visit)
cvd_association$cvd_bsln <- ifelse(
  cvd_association$cvd_dxdt < cvd_association$dt_visit0,
  1, 0
)

# Step 3: Exclude participants with baseline CVD
incident_data <- cvd_association %>%
  filter(cvd_bsln == 0)

# Step 4: Define incident CVD and censoring
incident_data <- incident_data %>%
  mutate(
    incident_cvd_time = cvd_dxdt,  # First CVD diagnosis after baseline
    event = ifelse(!is.na(incident_cvd_time) & incident_cvd_time != Inf, 1, 0),  # Event indicator: 1 if incident CVD occurred
censor_time = pmin(as.numeric(as.Date(f.40000.0.0)), as.numeric(as.Date(f.191.0.0)), as.numeric(as.Date("2022-11-30")), na.rm = TRUE),  # Censor at death or current date
    time_to_event = ifelse(event == 1, incident_cvd_time - dt_visit0, censor_time - dt_visit0)  # Time to event or censoring
  )

# Treat CKM stage as a categorical variable for Model 4
incident_data$ckm <- as.factor(incident_data$ckm)

# Check the event distribution
print("Baseline and incident CVD have been defined successfully.")
print(table(incident_data$event))  # Check event distribution

# Save the processed dataset for downstream analysis
#processed_output <- "/n/home_fasse/txia/UKB_CKM/Data/processed_cvd_data.csv"
#write.table(incident_data, file = processed_output, row.names = FALSE, quote = FALSE, col.names = TRUE, sep = ",")

#print("Processed CVD data saved successfully.")

# Load individual proteins with non-zero coefficients
important_proteins <- fread("/n/home_fasse/txia/UKB_CKM/Data/1vs0_selected_proteins_with_info_bootstrap.csv")
...
# Extract results for all models
  ...
  write.csv(
    annotated_results,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/cvd_ckm1vs0_association_threshold_", threshold, ".csv"),
    row.names = FALSE
  )
  ...
  write.csv(
    bonferroni_results,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/Significant_ckm1vs0_Model3_Bonferroni_Threshold_", threshold, ".csv"),
    row.names = FALSE
  )
  ...
  write.csv(
    fdr_results,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/Significant_ckm1vs0_Model3_FDR_Threshold_", threshold, ".csv"),
    row.names = FALSE
  )
  ...
  # Save Model 4 Bonferroni < 0.05 results
  bonferroni_results_model4 <- model4_results %>% filter(Bonferroni_P < 0.05)
  write.csv(
    bonferroni_results_model4,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/Significant_ckm1vs0_Model4_Bonferroni_Threshold_", threshold, ".csv"),
    row.names = FALSE
  )
  
  # Save Model 4 FDR < 0.05 results
  fdr_results_model4 <- model4_results %>% filter(FDR < 0.05)
  write.csv(
    fdr_results_model4,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/Significant_ckm1vs0_Model4_FDR_Threshold_", threshold, ".csv"),
    row.names = FALSE
  )
  ...
  mutate(
    ...
    for (i in 1:4) {
      if (i == 1) {
        ...
        model <- coxph(Surv(time_to_event, event) ~ get(tertile_var) + age.y + sex.y + nonwhite +fast_hrs+ townsend  + PA  + alcohol_cat, data = data)
      } else if (i == 3) {
        ...
        model <- coxph(Surv(time_to_event, event) ~ get(tertile_var) + age.y + sex.y + nonwhite +fast_hrs+ townsend  + PA  + alcohol_cat + ckm, data = data)
      }
...
# Set ckm for Model 4 spline predictions
if (is.factor(data$ckm)) {
  new_data$ckm <- factor(rep(levels(data$ckm)[1], 100), levels = levels(data$ckm))
} else {
  new_data$ckm <- median(as.numeric(data$ckm), na.rm = TRUE)
}
...
new_data$HR_upper <- exp(spline_pred$fit + 1.96 * spline_pred$se.fit)
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_ckm1vs0_", score_var, "_model3_", label, ".csv"))

    # Fit and save Model 4 spline model (Model 2 + ckm)
    ...
    fwrite(new_data_model4, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_ckm1vs0_", score_var, "_model4_", label, ".csv"))
  }
}
...
write.csv(
  ...
  "/n/home_fasse/txia/UKB_CKM/Data/cvd_ckm1vs0_proteomics_score_all_models_all_thresholds_stratified.csv",
  row.names = FALSE
)

write.csv(
  all_tertile_results,
  "/n/home_fasse/txia/UKB_CKM/Data/cvd_ckm1vs0_proteomics_score_tertile_models_all_thresholds_stratified.csv",
  row.names = FALSE
)

print("Completed all models for all protein score thresholds including spline models and Model 4 (Model 2 + ckm), stratified by drug use.")
