...
important_proteins <- fread("/n/home_fasse/txia/UKB_CKM/Data/polr_selected_proteins_with_info.csv")

# Merge protein annotation with selected proteins
merged_proteins <- merge(
  important_proteins,
  proteomics_info,          # must contain a 'UniProt' column matching 'Protein'
  by.x = "Protein",
  by.y = "UniProt",
  all.x = TRUE
)
...
for (threshold in thresholds) {
  ...
  # Keep only proteins present in the dataset
  ...
  # Association with individual proteins (PER SD)
  protein_results <- data.frame()

  for (protein in valid_proteins) {
    # ---- per-SD standardization for this protein ----
    zcol <- paste0(".__z_", make.names(protein))  # temp safe column name
    x    <- incident_data[[protein]]
    mu   <- mean(x, na.rm = TRUE)
    sg   <- sd(x,   na.rm = TRUE)
    if (!is.finite(sg) || sg == 0) next  # skip constant/singular proteins
    incident_data[[zcol]] <- as.numeric((x - mu) / sg)

    # Build formulas using the z-scored column
    f1 <- as.formula(sprintf("Surv(time_to_event, event) ~ `%s` + age.y + sex.y", zcol))
    f2 <- as.formula(sprintf("Surv(time_to_event, event) ~ `%s` + age.y + sex.y + nonwhite +fast_hrs + townsend + smoke + PA + BMI + alcohol_cat", zcol))
    f3 <- as.formula(sprintf("Surv(time_to_event, event) ~ `%s` + age.y + sex.y + nonwhite +fast_hrs + townsend + smoke + PA + BMI + alcohol_cat +sbp_use +  f.30690.0.0.x + f.30760.0.0.x +  f.30740.0.0.x + eGFR", zcol))

    # Fit Cox models
    model1 <- try(coxph(f1, data = incident_data), silent = TRUE)
    model2 <- try(coxph(f2, data = incident_data), silent = TRUE)
    model3 <- try(coxph(f3, data = incident_data), silent = TRUE)
    ...
    for (i in seq_along(models)) {
      ...
      s <- summary(model)
      # term is exactly the z-scored column
      est <- s$coef[zcol, "coef"]; se <- s$coef[zcol, "se(coef)"]; p_val <- s$coef[zcol, "Pr(>|z|)"]
      hr <- exp(est)
    ...
    # (optional) clean up the temp z column if you want:
    # incident_data[[zcol]] <- NULL
  }

  if (!nrow(protein_results)) {
    cat("No model results at threshold:", threshold, "\n")
    next
  }

  # Apply Bonferroni and FDR adjustments (within each Model)
  ...
  # Merge with annotation information (keeps your original screening 'Coefficient' too)
  ...
  write.csv(
    annotated_results,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/overallscore_cvd_association_threshold_", threshold, ".csv"),
    row.names = FALSE
  )

  # Extract Model 3 significant results
  model3_results <- annotated_results %>% dplyr::filter(Model == "Model 3")

  # Save Model 3 Bonferroni < 0.05 results
  write.csv(
    model3_results %>% dplyr::filter(Bonferroni_P < 0.05),
    paste0("/n/home_fasse/txia/UKB_CKM/Data/Significant_Model3_Bonferroni_Threshold_", threshold, ".csv"),
    row.names = FALSE
  )

  # Save Model 3 FDR < 0.05 results
  write.csv(
    model3_results %>% dplyr::filter(FDR < 0.05),
    paste0("/n/home_fasse/txia/UKB_CKM/Data/Significant_overallscore_Model3_FDR_Threshold_", threshold, ".csv"),
    row.names = FALSE
  )
...
new_data$HR_upper <- exp(spline_pred$fit + 1.96 * spline_pred$se.fit)
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_cvd_overallscore_", score_var, "_model3_", label, ".csv"))
  }
}
...
write.csv(
  ...
  "/n/home_fasse/txia/UKB_CKM/Data/cvd_overallscore_all_models_all_thresholds_stratified.csv",
  row.names = FALSE
)

write.csv(
  all_tertile_results,
  "/n/home_fasse/txia/UKB_CKM/Data/cvd_overallscore_tertile_models_all_thresholds_stratified.csv",
  row.names = FALSE
)
