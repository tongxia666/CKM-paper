...
table(cvd_association$sex.y)
        ...
        cvd_association$protein_score<-cvd_association$manual_protein_score_top
...
new_data$HR_upper <- exp(spline_pred$fit + 1.96 * spline_pred$se.fit)
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_cvd_overallscoretop_", score_var, "_model3_", label, ".csv"))
  }
}
...
write.csv(
  ...
  "/n/home_fasse/txia/UKB_CKM/Data/cvd_overallscoretop_all_models_all_thresholds_stratified.csv",
  row.names = FALSE
)

write.csv(
  all_tertile_results,
  "/n/home_fasse/txia/UKB_CKM/Data/cvd_overallscoretop_tertile_models_all_thresholds_stratified.csv",
  row.names = FALSE
)
