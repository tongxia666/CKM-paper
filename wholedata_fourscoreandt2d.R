...
new_data$eGFR <- median(as.numeric(data$eGFR), na.rm = TRUE)
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_t2d_alldata_", score_var, "_model3_", label, ".csv"))
  }
}
...
fwrite(all_proteomics_score_results, "/n/home_fasse/txia/UKB_CKM/Data/t2d_alldata_all_scores_all_models.csv")
fwrite(all_tertile_results, "/n/home_fasse/txia/UKB_CKM/Data/t2d_alldata_all_scores_tertile_models.csv")
...
for (threshold in thresholds) {
  ...
  for (ct in names(merged_list)) {
    ...
    write.csv(
      annotated_results,
      sprintf("/n/home_fasse/txia/UKB_CKM/Data/wholedata_t2d_ckm%s_association_threshold_%s.csv", ct, threshold),
      row.names = FALSE
    )
    ...
    write.csv(
      ...
      sprintf("/n/home_fasse/txia/UKB_CKM/Data/wholedata_t2d_Significant_ckm%s_Model3_Bonferroni_Threshold_%s.csv", ct, threshold),
      row.names = FALSE
    )
    write.csv(
      model3_res %>% filter(FDR < 0.05),
      sprintf("/n/home_fasse/txia/UKB_CKM/Data/wholedata_t2d_Significant_ckm%s_Model3_FDR_Threshold_%s.csv", ct, threshold),
      row.names = FALSE
    )
  ...
  if (length(all_contrasts_results)) {
    ...
    write.csv(
      combined,
      sprintf("/n/home_fasse/txia/UKB_CKM/Data/wholedata_t2d_ckm_allcontrasts_association_threshold_%s.csv", threshold),
      row.names = FALSE
    )
  } else {
