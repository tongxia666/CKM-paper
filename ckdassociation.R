...
# Load the dataset
ckd_association <- fread("/n/home_fasse/txia/UKB_CKM/Data/1_vs_0_combined_training_optimization_testing.csv")
...
ckd_association[, protein_score_0.2 := qnorm(rank(protein_score_0.2, na.last = "keep") / (.N + 1))]

summary(ckd_association$protein_score)
sd(ckd_association$protein_score, na.rm = TRUE)


summary(ckd_association$protein_score_0.2)
sd(ckd_association$protein_score_0.2, na.rm = TRUE)




# 1. Basic summary
summary_stats <- summary(ckd_association$protein_score)
...
# 3. Identify outliers
outliers <- ckd_association$protein_score[
  ckd_association$protein_score < lower_bound |
  ckd_association$protein_score > upper_bound
]

# 4. Compute 1st and 99th percentiles
p1 <- quantile(ckd_association$protein_score, 0.01, na.rm = TRUE)
p99 <- quantile(ckd_association$protein_score, 0.99, na.rm = TRUE)
...
# Extract results for all models
  ...
  write.csv(
    annotated_results,
    paste0("/n/home_fasse/txia/UKB_CKM/Data/ckd_ckm1vs0_association_threshold_", threshold, ".csv"),
    row.names = FALSE
  )
...
new_data$HR_upper <- exp(spline_pred$fit + 1.96 * spline_pred$se.fit)
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_ckd_ckm1vs0_", score_var, "_model3_", label, ".csv"))
    ...
    fwrite(new_data_model4, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_ckd_ckm1vs0_", score_var, "_model4_", label, ".csv"))
  }
}
...
write.csv(
  ...
  "/n/home_fasse/txia/UKB_CKM/Data/ckd_ckm1vs0_proteomics_score_all_models_all_thresholds_stratified.csv",
  row.names = FALSE
)

write.csv(
  all_tertile_results,
  "/n/home_fasse/txia/UKB_CKM/Data/ckd_ckm1vs0_proteomics_score_tertile_models_all_thresholds_stratified.csv",
  row.names = FALSE
)
