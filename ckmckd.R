...
for (ckm_comp in ckm_comparisons) {
  ...
  for (i in 1:3) {
    ...
    ckm_ckd_results <- rbind(ckm_ckd_results, data.frame(
...
ckm_ckd_results <- ckm_ckd_results %>%
  group_by(Model) %>%
  mutate(
...
# Save results
write.csv(ckm_ckd_results, "/n/home_fasse/txia/UKB_CKM/Data/ckmckdassociation.csv", row.names = FALSE)
print("CKM vs. ckd associations saved.")
...
# Define function
run_ckm_vs_ckd <- function(data, group_label) {
  ...
  # Save
  write.csv(ckm_ckd_results,
            paste0("/n/home_fasse/txia/UKB_CKM/Data/ckmckdassociation_", group_label, ".csv"),
            row.names = FALSE)
  
  print(paste0("Finished CKM vs. ckd associations for ", group_label))
}

# Run separately
run_ckm_vs_ckd(incident_data_nodrug, "nodrug")
run_ckm_vs_ckd(incident_data_drug, "drug")
