...
# Identify first cvd diagnosis date and handle Inf values
...
# Determine baseline cvd
cvd_association$cvd_bsln <- ifelse(
  cvd_association$cvd_dxdt < cvd_association$dt_visit0,
  1, 0
)

# Exclude participants with baseline cvd
incident_data <- cvd_association %>% filter(cvd_bsln == 0)

# Define incident cvd and censoring
...
run_models_on_subset <- function(data, label) {
  ...
  for (score_var in protein_scores) {
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_cvd_overallscore_alldata_", score_var, "_model3_", label, ".csv"))
  }
}
...
fwrite(all_proteomics_score_results, "/n/home_fasse/txia/UKB_CKM/Data/cvd_alldata_all_overallscores_all_models.csv")
fwrite(all_tertile_results, "/n/home_fasse/txia/UKB_CKM/Data/cvd_alldata_all_overallscores_tertile_models.csv")
