...
# Check for NA values in selected proteins for 4 vs 3 comparison
na_proteins_check <- sapply(selected_proteins_4_vs_3, function(col) any(is.na(ukb[[col]])))
if (any(na_proteins_check)) {
  ...
  print("No NA values found in the selected proteins for 4 vs 3 comparison.")
}

# Filter the dataset to include only rows where ckm is 2 or 0 for the `4 vs 3` comparison
comparison_column <- "ckm"  # Specify the column for ckm stages

print("Filtering dataset for 4 vs 3 comparison...")
ukb <- ukb[ckm %in% c(3, 4), ]  # Only include rows with ckm 0 and 1
...
for (i in 1:bootstrap_iterations) {
  print(i)
  x_sample <- as.matrix(training_set[jj[[i]], ..selected_proteins_4_vs_3])
...
prot.select <- matrix(NA, nrow = length(selected_proteins_4_vs_3) + 1, ncol = bootstrap_iterations)
for (i in 1:bootstrap_iterations) {
...
write.csv(merged_result, "/n/home_fasse/txia/UKB_CKM/Data/4vs3_selected_proteins_with_info_bootstrap.csv", row.names = FALSE)
...
for (threshold in thresholds) {
  ...
  out_path <- paste0("/n/home_fasse/txia/UKB_CKM/Data/4vs3_selected_proteins_coeffs_threshold_", threshold, ".csv")
...
#########get auc
roc_protein_score_0.1 <- roc(as.numeric(as.character(y_test)), testing_set$protein_score_0.1)
auc_protein_score_0.1 <- auc(roc_protein_score_0.1)
print(paste("AUC of auc_protein_score_0.1 :", round(auc_protein_score_0.1 , 4)))


roc_protein_score_0.15 <- roc(as.numeric(as.character(y_test)), testing_set$protein_score_0.15)
auc_protein_score_0.15 <- auc(roc_protein_score_0.15)
print(paste("AUC of auc_protein_score_0.15 :", round(auc_protein_score_0.15 , 4)))
...
write.csv(combined_data, "/n/home_fasse/txia/UKB_CKM/Data/4_vs_3_combined_training_optimization_testing.csv", row.names = FALSE)
...
# Save
write.csv(final_output_table, "/n/home_fasse/txia/UKB_CKM/Data/4vs3_selected_proteins_coeffandintercept_with_info_bootstrap.csv", row.names = FALSE)
...
if (threshold == 0.2) {
  ...
  write.csv(final_output_table_0.2, "/n/home_fasse/txia/UKB_CKM/Data/4vs3_selected_proteins_coeffandintercept_with_info_bootstrap_coef0.2_from_refit.csv", row.names = FALSE)
...
# Model 3: Covariates + Proteomics score_0.1
formula_covariates_proteins_0.1 <- as.formula(paste("ckm ~", paste(c(covariates, "protein_score_0.1"), collapse = " + ")))
model_covariates_proteins_0.1 <- glm(formula_covariates_proteins_0.1, data = optimization_set, family = binomial)
predicted_prob_covariates_proteins_0.1 <- predict(model_covariates_proteins_0.1, newdata = testing_set, type = "response")
roc_covariates_proteins_0.1 <- roc(as.numeric(as.character(testing_set$ckm)), predicted_prob_covariates_proteins_0.1)


# Model 4: Covariates + Proteomics score_0.15
formula_covariates_proteins_0.15 <- as.formula(paste("ckm ~", paste(c(covariates, "protein_score_0.15"), collapse = " + ")))
model_covariates_proteins_0.15 <- glm(formula_covariates_proteins_0.15, data = optimization_set, family = binomial)
predicted_prob_covariates_proteins_0.15 <- predict(model_covariates_proteins_0.15, newdata = testing_set, type = "response")
roc_covariates_proteins_0.15 <- roc(as.numeric(as.character(testing_set$ckm)), predicted_prob_covariates_proteins_0.15)
...
# ---------- Basic AUC Plot ----------
pdf("/n/home_fasse/txia/UKB_CKM/Data/Combined_proteomicscore_covariate_ROC_Curves_4_vs_3.pdf", width = 18, height = 8)
...
# ---------- AUC with 95% CI Plot ----------
pdf("/n/home_fasse/txia/UKB_CKM/Data/Combined_proteomicscore_covariate_ROC_Curves_with_CI_legend_4_vs_3.pdf", width = 18, height = 8)
legend_labels <- c()
