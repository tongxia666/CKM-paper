...
# Identify first t2d diagnosis date and handle Inf values
...
# Determine baseline t2d
t2d_association$t2d_bsln <- ifelse(
  t2d_association$t2d_dxdt < t2d_association$dt_visit0,
  1, 0
)

# Exclude participants with baseline t2d
incident_data <- t2d_association %>% filter(t2d_bsln == 0)

# Define incident t2d and censoring
...
run_models_on_subset <- function(data, label) {
  ...
  for (score_var in protein_scores) {
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_t2d_overallscore_alldata_", score_var, "_model3_", label, ".csv"))
  }
}
...
fwrite(all_proteomics_score_results, "/n/home_fasse/txia/UKB_CKM/Data/t2d_alldata_all_overallscores_all_models.csv")
fwrite(all_tertile_results, "/n/home_fasse/txia/UKB_CKM/Data/t2d_alldata_all_overallscores_tertile_models.csv")
