...
for (ckm_comp in ckm_comparisons) {
  ...
  for (i in 1:3) {
    ...
    ckm_cvd_results <- rbind(ckm_cvd_results, data.frame(
...
ckm_cvd_results <- ckm_cvd_results %>%
  group_by(Model) %>%
  mutate(
...
# Save results
write.csv(ckm_cvd_results, "/n/home_fasse/txia/UKB_CKM/Data/ckmcvdassociation.csv", row.names = FALSE)
print("CKM vs. CVD associations saved.")
...
# Define function
run_ckm_vs_cvd <- function(data, group_label) {
  ...
  # Save
  write.csv(ckm_cvd_results,
            paste0("/n/home_fasse/txia/UKB_CKM/Data/ckmcvdassociation_", group_label, ".csv"),
            row.names = FALSE)
  
  print(paste0("Finished CKM vs. CVD associations for ", group_label))
}

# Run separately
run_ckm_vs_cvd(incident_data_nodrug, "nodrug")
run_ckm_vs_cvd(incident_data_drug, "drug")
