...
t2d_association$protein_score<-t2d_association$manual_protein_score_top
...
new_data$HR_upper <- exp(spline_pred$fit + 1.96 * spline_pred$se.fit)
    ...
    fwrite(new_data, paste0("/n/home_fasse/txia/UKB_CKM/Data/spline_t2d_overallscoretop_", score_var, "_model3_", label, ".csv"))
  }
}
...
write.csv(
  ...
  "/n/home_fasse/txia/UKB_CKM/Data/t2d_overallscoretop_all_models_all_thresholds_stratified.csv",
  row.names = FALSE
)

write.csv(
  all_tertile_results,
  "/n/home_fasse/txia/UKB_CKM/Data/t2d_overallscoretop_tertile_models_all_thresholds_stratified.csv",
  row.names = FALSE
)
