...
# Check for NA values in selected proteins for 1 vs 0 comparison
na_proteins_check <- sapply(selected_proteins_1_vs_0, function(col) any(is.na(ukb[[col]])))
if (any(na_proteins_check)) {
  ...
  print("No NA values found in the selected proteins for 1 vs 0 comparison.")
}

# Filter the dataset to include only rows where ckm is 2 or 0 for the `1 vs 0` comparison
comparison_column <- "ckm"  # Specify the column for ckm stages

print("Filtering dataset for 1 vs 0 comparison...")
ukb <- ukb[ckm %in% c(0,1), ]  # Only include rows with ckm 0 and 1
...
for (i in 1:bootstrap_iterations) {
  print(i)
  x_sample <- as.matrix(training_set[jj[[i]], ..selected_proteins_1_vs_0])
...
prot.select <- matrix(NA, nrow = length(selected_proteins_1_vs_0) + 1, ncol = bootstrap_iterations)
for (i in 1:bootstrap_iterations) {
...
write.csv(merged_result, "/n/home_fasse/txia/UKB_CKM/Data/1vs0_selected_proteins_with_info_bootstrap.csv", row.names = FALSE)
...
for (threshold in thresholds) {
  ...
  out_path <- paste0("/n/home_fasse/txia/UKB_CKM/Data/1vs0_selected_proteins_coeffs_threshold_", threshold, ".csv")
...
write.csv(combined_data, "/n/home_fasse/txia/UKB_CKM/Data/1_vs_0_combined_training_optimization_testing.csv", row.names = FALSE)
...
# Save
write.csv(final_output_table, "/n/home_fasse/txia/UKB_CKM/Data/1vs0_selected_proteins_coeffandintercept_with_info_bootstrap.csv", row.names = FALSE)
...
if (threshold == 0.2) {
  ...
  write.csv(final_output_table_0.2, "/n/home_fasse/txia/UKB_CKM/Data/1vs0_selected_proteins_coeffandintercept_with_info_bootstrap_coef0.2_from_refit.csv", row.names = FALSE)
...
# ---------- Basic AUC Plot ----------
pdf("/n/home_fasse/txia/UKB_CKM/Data/Combined_proteomicscore_covariate_ROC_Curves_1_vs_0.pdf", width = 18, height = 8)
...
# ---------- AUC with 95% CI Plot ----------
pdf("/n/home_fasse/txia/UKB_CKM/Data/Combined_proteomicscore_covariate_ROC_Curves_with_CI_legend_1_vs_0.pdf", width = 18, height = 8)
legend_labels <- c()
